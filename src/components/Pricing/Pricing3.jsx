...
const Pricing3 = () => {
  return (
    <section className="p-4 md:p-8 dark:bg-gray-900 dark:text-gray-100">
      <div className="py-8 max-w-6xl mx-auto lg:py-16">
        <div className="mx-auto max-w-3xl text-center pb-12 md:pb-20">
          <h2 className="text-3xl font-bold sm:text-4xl mb-4 dark:text-gray-200">Pricing Plans</h2>
          <p className="text-xl dark:text-gray-400">Choose a plan that best suits your data needs.</p>
        </div>
        <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-10 lg:space-y-0">
          {pricingPlans.map((plan, index) => (
            ...
            >
              ...
              <ul role="list" className="mb-8 space-y-4 text-left dark:text-gray-300">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center space-x-3">
                    <svg
                      className={`w-6 h-6 ${idx < 5 ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}
                      aria-hidden="true"
              ...
              <button
                className={`w-full font-bold gap-2 shadow uppercase p-2 text-white ${plan.buttonClass}`}
              >
                {plan.buttonText}
              </button>
            </div>
